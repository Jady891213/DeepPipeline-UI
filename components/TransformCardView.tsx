...
const TransformCardView: React.FC = () => {
  ...
  return (
    <div className="flex flex-col h-full bg-white">
      ...
      {/* Card List Area */}
      <div className="flex-1 overflow-y-auto p-2.5 space-y-2.5 bg-slate-50/20">
        {filteredRules.map(rule => {
          ...
          return (
            <div key={rule.id} className="bg-white border border-slate-200 rounded-lg shadow-sm group hover:border-indigo-300 hover:shadow-md transition-all overflow-hidden flex flex-col">
              ...
              {/* Header: Source Selection & Actions */}
              <div className="px-2 py-1.5 border-b border-slate-50 flex items-center justify-between bg-slate-50/20">
                ...
                <div className="flex items-center gap-0.5 ml-2">
                  <button 
                    ...
                    title="点击设置目标名称"
                    className={`p-1 rounded transition-colors ${isRenaming ? 'bg-indigo-100 text-indigo-600' : 'text-slate-400 hover:text-indigo-600 hover:bg-white'}`}
                  >
                    <Edit3 size={11} />
                  </button>
                  <button onClick={() => copyRule(rule)} className="p-1 text-slate-400 hover:text-indigo-600 hover:bg-white rounded transition-colors"><Copy size={11} /></button>
                  <button onClick={() => removeRule(rule.id)} className="p-1 text-slate-400 hover:text-red-500 hover:bg-white rounded transition-colors"><Trash2 size={11} /></button>
                </div>
              </div>

              {/* Body: High-Density Input Group */}
              <div className="p-2 space-y-2">
                
                {/* Row 1: Conditional Target Name Input */}
                {isRenaming && (
                  <div className="relative animate-in slide-in-from-top-1 duration-200">
                    <Type size={10} className="absolute left-2.5 top-1/2 -translate-y-1/2 text-indigo-400" />
                    <input 
                      type="text"
                      value={rule.targetName}
                      onChange={(e) => updateRule(rule.id, { targetName: e.target.value })}
                      className="w-full text-[11px] font-bold text-indigo-700 bg-indigo-50/30 border border-indigo-100 rounded pl-7 pr-7 py-1 outline-none focus:border-indigo-400"
                      placeholder="设置目标字段名..."
                      autoFocus
                    />
                    <button 
                ...
                {/* Row 2: Type (30%) & Configuration (70%) */}
                <div className="flex gap-1.5">
                  <div className="w-[35%]">
                    <select 
                      value={rule.targetType}
                      onChange={(e) => updateRule(rule.id, { targetType: e.target.value as ColumnType })}
                      className="w-full text-[10px] font-bold text-slate-600 border border-slate-200 rounded px-1.5 py-1 bg-white outline-none focus:border-indigo-400 transition-colors cursor-pointer"
                    >
                      {TYPE_OPTIONS.map(opt => <option key={opt.value} value={opt.value}>{opt.label}</option>)}
                    </select>
                  </div>

                  <div className={`flex-1 relative ${!typeConfig?.hasFormat ? 'opacity-30 grayscale pointer-events-none' : ''}`}>
                    ...
                    <input 
                      ...
                      className="w-full text-[10px] font-mono text-slate-600 bg-slate-50/50 border border-slate-200 rounded pl-6 pr-2 py-1 outline-none focus:border-indigo-400 focus:bg-white transition-all"
                      placeholder={typeConfig?.hasFormat ? "设置转换格式 (如 YYYY-MM-DD)" : "无须转换参数"}
                    />
                  </div>
                </div>
              </div>
      ...
      <div className="px-3 py-1 bg-slate-50 border-t border-slate-100 flex justify-between items-center shrink-0">
        <span className="text-[9px] font-bold text-slate-400 uppercase tracking-tighter">Fields: {filteredRules.length}</span>
        <span className="text-[9px] font-bold text-indigo-400 uppercase tracking-tighter cursor-pointer hover:underline">批量设置</span>
      </div>
    </div>
  );
};
